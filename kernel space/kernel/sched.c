...
const int INT_MULTIPLIER = 1000000;
extern int schedValue;
...
asmlinkage void schedule(void)
{
	...
	int this_cpu, c, lowestPass;
//	int INT_MULTIPLIER = 1000;
	...
	if(schedValue == 1){
		...
		*/
    		...
    		if (selected == NULL) {
        		...
        		read_lock(&tasklist_lock);
        		for_each_task(p)
            			p->pass -= INT_MULTIPLIER;
        		read_unlock(&tasklist_lock);
        		spin_lock_irq(&runqueue_lock);
        		goto repeat_schedule;
    		}

    		selected->pass += (INT_MULTIPLIER / selected->ticket);
    		next = selected;
	}
